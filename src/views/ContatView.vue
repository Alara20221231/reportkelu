<template>
    <div class="contact">
        <div class="container mt-5 mb-5" id="ViewCard">
            <div class="row">
                <div class="col-12 col-sm-6 mx-auto z-1 mt-5">
  ...
  methods:{
    handleSubmit(){
        ...
        if(name==""||call==""||email==""||subject==""||message==""){
            ...
            var data = {
            'entry.18572279': name,
            'entry.1700150167': call,
            'entry.826752828': email,
            'entry.1539584672': subject,
            'entry.1363105570':message,
            };
            $.ajax({
            type: 'POST',
            url: 'https://docs.google.com/forms/u/0/d/e/1FAIpQLSfTQ-6e5_xtKxMR9hY4I-Af1vzLIEjHgq56w0nsmtsP8ePMgg/formResponse',
            data: data,
            contentType: 'application/json',
            dataType: 'jsonp',
            complete: function() {
                $('.thank').text('謝謝您的留言我已收到!我們將盡快回覆於您所留存的電子郵件');
                $('.ok').removeClass('d-none');
                $('.ok').addClass('d-block');
                $('.viewback').removeClass('d-none');
                $('.viewback').addClass('show');
